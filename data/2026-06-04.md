...
TL;DR: SceneDiver先理解整个场景，再逐步缩小注意范围到关键物体，从而减少VLM/VLA在机器人任务中的视觉幻觉；同时它还能把这种聚焦能力高效迁移到VLA。 / SceneDiver first understands the whole scene and then progressively narrows attention to critical objects, reducing visual hallucinations in VLM/VLA-based robotic tasks; this focusing ability is also efficiently transferred to VLAs.


<details>
  <summary>Details</summary>
Motivation: 动机在于VLM和VLA虽然分别擅长长期规划和反应式控制，但都受制于相同的感知瓶颈：由于无法可靠地区分任务相关物体与干扰项，容易产生视觉幻觉。直接一步式聚焦无效，因为有效聚焦依赖更深层的场景理解。 / The motivation is that although VLMs and VLAs excel at long-term planning and reactive control respectively, both suffer from the same perceptual bottleneck: visual hallucinations caused by poor discrimination between task-relevant objects and distractors. A direct one-step focus strategy is ineffective because effective focus requires deeper scene understanding.

Method: 方法上，作者提出SceneDiver：先构建整体场景图形成全局理解，再通过识别、理解、分析的迭代循环，将任务逐步分解为更简单的子问题，生成粗到细的聚焦计划；同时设计轻量适配器，将这种审慎聚焦能力蒸馏到VLA以支持反应式控制。 / Methodologically, the paper proposes SceneDiver: it first builds a holistic scene graph for global understanding, then iteratively performs recognition, understanding, and analysis to decompose the task into simpler sub-problems and generate a coarse-to-fine focus plan; additionally, a lightweight adapter distills this deliberate focusing ability into VLAs for reactive control.

Result: 在标准具身AI基准上的实验表明，该方法对VLM和VLA都能显著减少视觉幻觉，并在需要快速执行的任务中保持计算效率。 / Experiments on standard embodied AI benchmarks show that the method substantially reduces visual hallucinations for both VLMs and VLAs while maintaining computational efficiency in tasks requiring fast execution.

Conclusion: SceneDiver通过粗到细的聚焦规划显著降低了具身视觉-语言决策中的视觉幻觉，并且其聚焦能力还能以轻量方式蒸馏到VLA中，在保持快速执行效率的同时提升性能。 / SceneDiver significantly reduces visual hallucinations in embodied vision-language decision-making through coarse-to-fine focus planning, and its focus capability can be lightly distilled into VLAs, improving performance while preserving fast execution efficiency.
...
TL;DR: SASA improves weakly incremental semantic segmentation by anchoring class semantics with learnable tokens and cleaning weak labels with geometry-aware arbitration, leading to more stable learning and better benchmark performance than previous methods.


<details>
  <summary>Details</summary>
Motivation: Weakly incremental learning for semantic segmentation suffers from noisy supervision introduced over time, which corrupts class representations, causes feature drift and semantic corruption, and leads to catastrophic overwriting of previously learned classes. The work aims to make incremental semantic learning more stable and supervision more reliable.

Method: The paper proposes SASA with two main components: (1) Semantic Anchors, which use learnable class-level tokens as stable references, combined with elastic residual adaptation for instance-specific refinement; and (2) Spatial Label Arbitration, a geometry-aware supervision mechanism that filters unreliable labels and enforces a one-object-one-class constraint.

Result: Extensive experiments on standard benchmarks show that SASA consistently surpasses existing state-of-the-art weakly incremental semantic segmentation methods, with particularly strong gains in difficult multi-step incremental scenarios.

Conclusion: SASA is a drift-resilient weakly incremental semantic segmentation method that stabilizes semantic learning through Semantic Anchors and Spatial Arbitration, effectively reducing feature drift and semantic corruption under weak supervision and outperforming prior methods, especially in multi-step incremental settings.
...
TL;DR: IN2R improves noisy cross-modal retrieval by building soft supervision targets from stable intra-modal neighbor structure instead of relying on one discrete replacement label.


<details>
  <summary>Details</summary>
Motivation: Web-harvested cross-modal retrieval datasets contain noisy image-text correspondences. Prior methods mainly follow a discrete selection paradigm, such as filtering samples or choosing substitute labels, which can cause single-point fragility and discretization error.

Method: The paper proposes Intra-modal Neighbor-aware Noise Rectification (IN2R). It uses a Graph Refiner to reason over intra-modal neighbors retrieved from a dynamic Cross-Model Memory, then synthesizes a continuous soft prototype from local semantic consensus to rectify noisy inter-modal correspondence.

Result: Extensive experiments on Flickr30K, MS-COCO, and CC152K show that IN2R significantly outperforms state-of-the-art methods for cross-modal retrieval in noisy settings.

Conclusion: IN2R replaces discrete proxy selection with synthesized continuous soft prototypes derived from intra-modal neighbors, leading to more robust noise rectification and stronger cross-modal retrieval performance under noisy web data.
...
TL;DR: MeshFlow replaces slow autoregressive mesh generation with a VAE + Rectified Flow model that generates mesh geometry and connectivity in parallel, yielding compact representations, avoiding quantization, and delivering 18x faster inference with high quality.


<details>
  <summary>Details</summary>
Motivation: Prior autoregressive mesh generators are limited by quadratic inference cost with respect to mesh size and by the need to discretize vertex coordinates, which causes quantization errors. The work aims to build a faster and more accurate mesh generator that handles geometry and topology more naturally.

Method: The paper proposes MeshFlow: a VAE encodes continuous vertex positions and discrete mesh connectivity into a compact continuous latent space, trained with additional contrastive supervision. A Rectified Flow transformer then generates mesh vertices and edges in parallel from this latent representation.

Result: MeshFlow generates meshes 18x faster than the fastest autoregressive baseline and achieves strong performance on standard mesh-generation metrics, while representing meshes more compactly than token-based approaches.

Conclusion: MeshFlow demonstrates that continuous latent-space mesh generation with a VAE plus Rectified Flow can produce high-quality 3D meshes much faster than autoregressive methods, while avoiding coordinate quantization issues.
...
TL;DR: Choose a smarter prior instead of solving OT directly: use low-frequency image content as the prior, let flow matching add high-frequency details, and get straighter trajectories plus better few-step image generation.


<details>
  <summary>Details</summary>
Motivation: Existing OT-based flow matching benefits from straight, non-crossing trajectories, but exact OT coupling in high dimensions is intractable. Prior approximations either introduce bias or add substantial computational overhead. The paper is motivated by removing this bottleneck through a reformulation: if the prior is selectable, one can choose a tractable prior that naturally aligns with the data under identity coupling.

Method: The method reformulates flow matching by choosing a prior that admits an OT-optimal identity coupling with the data instead of computing an OT coupling between a fixed prior and data. Specifically, it uses the low-frequency projection of natural images as the prior, trains a lightweight sampler for that structured prior at inference, and lets the flow model synthesize the remaining high-frequency details. The method also interpolates this prior with Gaussian noise to improve quality while preserving the coupling properties.

Result: Across benchmarks, the proposed method reduces trajectory curvature by more than 2x relative to existing flow matching methods and improves generation quality in the few-step regime. It also integrates cleanly with latent models, classifier-free guidance, and one-step generation frameworks, while requiring no architectural change to the underlying flow model.

Conclusion: By redefining the prior as a design variable, the paper avoids explicitly solving high-dimensional optimal transport and still obtains near-OT flow trajectories. A low-frequency image prior, optionally mixed with Gaussian noise, produces straighter trajectories and better few-step generation without changing the flow model.
...
TL;DR: This paper builds EVID-Bench, a benchmark for detecting video misinformation that requires searching the web for related evidence rather than judging a video in isolation. Tests on nine strong multimodal models show poor performance, especially on AI-generated manipulations, highlighting that search-grounded cross-video verification is still a hard open problem.


<details>
  <summary>Details</summary>
Motivation: Many modern video misinformation cases cannot be verified from the input video alone because the manipulation depends on missing, reordered, replaced, or recontextualized evidence outside the video. Existing approaches that rely only on visual inspection are therefore insufficient.

Method: The paper introduces EVID-Bench, a benchmark for video misinformation detection that requires open-web search for related videos and cross-video comparison. It contains 222 videos, 9 manipulation types, and 3 categories: AI generation, single-source editing, and multi-source editing. The authors evaluate nine frontier multimodal models with a retrieval-augmented verification baseline.

Result: EVID-Bench samples are verified to be undetectable through visual inspection alone by frontier models. Across nine evaluated multimodal models, the best system achieves 61.43% point-level accuracy and 43.24% video-level accuracy. Major failure modes include fixation on irrelevant anchors, misclassification of synthetic content as splicing, and premature termination of search.

Conclusion: Current frontier multimodal models struggle with search-grounded video misinformation detection. On EVID-Bench, even the best retrieval-augmented system reaches only 61.43% point-level accuracy and 43.24% video-level accuracy, showing that evidence-dependent manipulations, especially AI-generated ones, remain far from solved.
...
TL;DR: 本文提出一个面向单图像反射分离的扩散模型框架，专门解决强眩光和弱反射等极端情况；核心是联合生成两层、跨层注意力、解耦采样和潜变量优化；实验上优于SOTA。 / This paper presents a diffusion-based framework for single-image reflection separation targeting extreme glare and weak-reflection cases; the key ideas are joint two-layer generation, cross-layer attention, disjoint sampling, and latent optimization; it outperforms SOTA in experiments.


<details>
  <summary>Details</summary>
Motivation: 动机在于，现有单图像反射分离方法在强眩光或弱反射场景下常因可用信息不足而无法同时准确恢复透射层和反射层，因此需要借助更强的生成先验来提升极端条件下的分离鲁棒性。 / The motivation is that existing single-image reflection separation methods often fail in glare-heavy or weak-reflection cases because the input contains insufficient information to reliably recover both layers, motivating the use of stronger generative priors for robust separation.

Method: 方法上，作者使用统一扩散模型同时生成透射层与反射层；引入跨层自注意力机制以增强两层特征解耦；提出解耦采样策略，在扩散过程中迭代降低层间干扰；并加入基于学习式合成函数的潜变量优化，以提升复杂真实场景中的重建质量。 / The method uses a unified diffusion model to jointly generate transmission and reflection layers, introduces cross-layer self-attention for feature disentanglement, applies a disjoint sampling strategy to iteratively suppress inter-layer interference during diffusion, and adds latent optimization with a learned composition function for better performance in complex real scenes.

Result: 实验结果表明，该方法在多个真实世界基准上全面超过现有最先进方法，说明其在极端反射条件下具有更好的泛化性与分离质量。 / Extensive experiments show that the method surpasses existing state-of-the-art approaches on multiple real-world benchmarks, indicating better generalization and separation quality under challenging reflection conditions.

Conclusion: 该方法通过针对单幅图像反射分离任务微调扩散模型，并结合跨层自注意力、解耦采样与潜变量优化，在强眩光和弱反射等极端场景下取得了更稳健的分离效果，整体性能优于现有最优方法。 / A diffusion model fine-tuned for single-image reflection separation, combined with cross-layer self-attention, disjoint sampling, and latent optimization, delivers more robust separation under extreme glare and weak-reflection conditions and outperforms prior state-of-the-art methods.
...
TL;DR: Pinpoint combines internet photos and street-view data in a shared retrieval-plus-reranking geolocation pipeline, improving global image geolocation accuracy and setting new state of the art without using multimodal LLMs.


<details>
  <summary>Details</summary>
Motivation: Worldwide image geolocation is difficult because visual cues are ambiguous, diverse, and unevenly distributed. Prior work usually separates internet-photo geolocation from street-view geolocation, missing the complementary benefits of these two data sources.

Method: The paper proposes a coarse-to-fine pipeline called Pinpoint. First, a contrastive image-GPS embedder is trained on both Flickr photos and street-view images to learn a shared image-GPS embedding space for candidate retrieval. Then, an attention-based reranker rescoring retrieved candidates combines visual and GPS features with cross-source evidence from nearby locations.

Result: Pinpoint reaches state-of-the-art results on all metrics across standard benchmarks for both internet photos (IM2GPS3k, YFCC4k) and street-view imagery (OSV-5M), while providing faster and more reproducible inference than methods relying on multimodal large-language models.

Conclusion: Pinpoint, a retrieve-and-rerank architecture that jointly uses internet photos and street-view imagery, achieves state-of-the-art geolocation performance on major benchmarks without relying on multimodal large-language models.
...
TL;DR: POLARIS通过“LLM裁判奖励+人类参考锚点注入”改进9B开源模型的长篇写作，使其以较低训练成本获得更好的长度遵循、故事质量和长度泛化能力。 / TL;DR: POLARIS boosts a 9B open-weight model for long-form story writing using LLM-judge rewards plus human-reference anchoring, yielding better length control, quality, and length generalization at relatively low compute cost.


<details>
  <summary>Details</summary>
Motivation: 论文动机是解决小型开源模型在长篇故事生成中的两个核心问题：一是无法达到用户要求的篇幅，二是随着篇幅增加文本质量明显下降，且相较前沿模型差距更大。作者希望用较低计算成本的方法提升长度控制与长文本质量。 / The motivation is to address two major weaknesses of small open-weight models in long-form story generation: they often fail to reach the requested length, and their quality degrades sharply as stories get longer, especially relative to frontier models. The goal is to improve long-context creative writing with modest compute.

Method: 方法上，作者基于GRPO提出POLARIS，包括两个核心组件：使用前沿LLM依据结构化故事质量量表进行在线奖励评估，以及在人类参考注入(HRI)中将教师强制的人类写作样本作为每个GRPO组内的高奖励锚点。该方法被应用于Qwen3.5-9B，使用约1.4K条提示-故事数据，在4张A100上训练。 / Methodologically, POLARIS is a lower-compute GRPO recipe with two key components: an online reward from a frontier LLM judge using a structured story-quality rubric, and Human-Reference Injection (HRI), where a teacher-forced human-written story is inserted as a high-reward anchor within each GRPO group. The recipe is applied to Qwen3.5-9B using about 1.4K prompt-story pairs on 4 A100 GPUs.

Result: 结果显示，POLARIS-9B在五个覆盖分布内外提示和不同评分标准的基准上具有竞争力，较基础Qwen3.5-9B明显更优，并在盲测人工评估中与Qwen3.5-27B相当。同时，它在训练样本仅到4k词的情况下，仍能较好泛化到最长约3倍训练长度的写作请求。 / Results show that POLARIS-9B is competitive with much larger open-weight models across five benchmarks, clearly outperforms base Qwen3.5-9B, and is on par with Qwen3.5-27B in blinded human evaluation. Despite training on stories up to 4k words, it maintains quality on prompts requesting up to roughly 3x that length.

Conclusion: 该论文提出的POLARIS训练方案显著提升了小型开源权重模型在长篇创意写作中的长度遵循能力与质量稳定性，使9B模型在多个基准上接近或达到更大模型的表现，并表明长度泛化是评估创意写作模型的重要压力测试。 / The paper concludes that POLARIS substantially improves long-form creative writing in small open-weight models, especially length adherence and quality retention, enabling a 9B model to approach larger models and showing that length generalization is a meaningful stress test for story-writing systems.
...
TL;DR: Same content, different wrapper labels, drastically different model behavior: labels like Instruction/Reference make models much more likely to follow misleading context, while Example reduces that tendency. Benchmarking context use without controlling these labels can therefore be seriously misleading.


<details>
  <summary>Details</summary>
Motivation: Many context-augmented LM systems wrap supplied content with discourse-role labels, but the influence of these labels on model reading behavior and reliance on context has not been systematically measured. This creates a risk that benchmark results reflect presentation choices rather than true context use.

Method: The paper builds a paired fixed-content probe on 500+ MMLU-Pro items, inserting the same misleading answer-bearing assertion under different labels (for example Instruction, Reference, Example) and measuring wrong-answer adoption across several models. It further uses paired statistical tests, bootstrap confidence intervals, final-instruction ablations, Qwen log-prob probes, boundary-condition experiments, and a 200-case manual audit for short-answer evaluation.

Result: Across GPT-5.5, DeepSeek V4 Pro, Llama-3-8B-Instruct, and Qwen2.5-7B-Instruct, misleading adoption shifts by 56 to 84 percentage points depending on the label. Binding or source-like labels such as Instruction and Reference strongly increase adoption, while Example suppresses it. The effect weakens on arithmetic tasks, persists in smaller form with passage-like context, is not explained by option-letter copying, and nested-label tests suggest illustrative framing limits adoption scope.

Conclusion: Wrapper labels materially change whether language models adopt misleading injected context; therefore context-utilization and reader-side RAG evaluations should explicitly report and control discourse-role labels such as Instruction, Reference, and Example.
...
TL;DR: This paper reviews how LLMs fit into the history of computational concept analysis in HPSS. It argues that LLMs are useful but not methodologically revolutionary: they enhance existing workflows while preserving major challenges in corpus building, modeling, and interpretation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to place current enthusiasm around LLMs in historical and methodological context. The authors aim to clarify what is genuinely new about LLMs for concept analysis in HPSS, what problems they inherit from previous computational methods, and how researchers should assess their opportunities and limitations.

Method: The paper uses a structured review and conceptual analysis. It first reconstructs pre-LLM computational conceptual history by synthesizing three research strands: early digital methods in HPSS, distributional approaches from digital history and related fields, and lexical semantic change detection. It then reviews LLM fundamentals, surveys LLM-based studies on lexical semantic change and HPSS case studies, and compares methodological issues across pre-LLM and LLM eras.

Result: The paper finds that LLMs provide new capabilities for concept analysis and semantic change research, but their practical use still depends on difficult methodological decisions. The review shows that challenges around data selection, model training background, task operationalization, and the reliability of interpretation continue to shape outcomes in both older and newer computational approaches.

Conclusion: The paper concludes that LLMs extend earlier computational approaches to conceptual history in HPSS, but they do not eliminate core methodological difficulties. Instead, longstanding issues such as corpus construction, operationalization, model selection, evaluation, and interpretation remain central and must be reconsidered in LLM-based workflows.
...
TL;DR: SALIMORY trains one LLM to handle structured conversational memory with decomposed rewards, solving memory credit assignment issues and substantially improving accuracy, failure rate, and personalization.


<details>
  <summary>Details</summary>
Motivation: Persistent memory is necessary for lifelong conversational agents, but naive context expansion with raw retrieval harms reasoning quality, and standard reinforcement learning for memory agents suffers from a severe credit assignment problem in multi-stage pipelines.

Method: The paper proposes SALIMORY, which uses a cognitively structured memory system covering user facts, preferences, and working memory. It trains a single model with a hierarchical stage-wise process reward and reward-decomposed contrastive refinement to separately supervise filtering, consolidation, and cue-driven recall.

Result: SALIMORY reduces memory-attributed failures by about one-third, exceeds the previous state of the art by more than 10% in end-to-end accuracy, and more than doubles the Good Personalization rate.

Conclusion: SALIMORY demonstrates that a single language model can be trained to manage cognitively structured long-term memory end-to-end, improving personalization and reducing memory-related failures compared with prior methods.
...
TL;DR: RAG helps much less than expected for medical QA; stronger backbone models matter more than better retrieval, suggesting evidence utilization is the main bottleneck.


<details>
  <summary>Details</summary>
Motivation: Medical QA is high-stakes, and RAG is commonly assumed to improve factual reliability. The paper revisits whether this assumption actually holds broadly across models, datasets, retrievers, and corpora.

Method: The paper systematically evaluates open-weight instruction-tuned models from 7B to 72B on ten biomedical QA datasets, comparing no-retrieval baselines against RAG setups using four retrieval methods and four retrieval corpora.

Result: Retrieval usually improves performance by only 1–2 points and does so inconsistently. Backbone model choice has a much larger impact than retriever or corpus choice, and expert versus layman retrieval sources perform similarly in most cases.

Conclusion: In medical QA, retrieval-augmented generation provides only small and inconsistent gains across many open-weight models; the larger limitation appears to be the model’s ability to use retrieved evidence rather than retrieval quality itself.
...
TL;DR: The paper shows that MoE LLM safety refusals can be bypassed with inference-time steering just like dense models. It introduces expert-aware steering methods, finds even one expert can carry a strong refusal signal, and argues that attention, not just routing, is important in MoE refusal behavior.


<details>
  <summary>Details</summary>
Motivation: Prior work showed that dense LLM refusal behavior can be suppressed by applying steering vectors at inference time, raising safety concerns. The authors investigate whether the same vulnerability exists in MoE architectures and whether MoE-specific internal structure can be exploited to steer refusal more effectively.

Method: The paper extends inference-time refusal steering from dense LLMs to three open-source Mixture-of-Experts LLMs, then proposes two expert-aware steering variants: one based on refusal-specific expert routing patterns and another based on expert-specific steering directions. The methods are evaluated by testing whether they can reduce normal refusal behavior on harmful or disallowed requests.

Result: The authors find that standard refusal steering works well on MoE models despite complex expert routing. Their expert-aware methods also suppress refusal effectively, and in some cases steering based on a single expert is sufficient. They further observe that steering signals do not align simply with expert routing behavior, suggesting a significant role for attention.

Conclusion: MoE LLMs remain vulnerable to refusal steering, and expert-aware steering can suppress refusal effectively, even using signals from a single expert. The steering signal appears to be distinct from expert routing alone, implying that attention mechanisms contribute substantially to refusal behavior in MoE models.
...
TL;DR: An optimized, explainable XGBoost model using only eight routine clinical measures can very accurately distinguish NC, MCI, and AD, with SHAP providing clinically plausible explanations of feature importance.


<details>
  <summary>Details</summary>
Motivation: There is a strong need for accurate and interpretable detection of normal cognition, mild cognitive impairment, and Alzheimer's disease from routine clinical assessments because AD affects over 55 million people globally and current clinical decision support remains inadequate.

Method: The study trained a three-class XGBoost classifier on ADNI baseline data using eight clinical variables (MMSE, CDR Global, CDR-SB, MoCA, FAQ, age, sex, education). Hyperparameters were optimized with Optuna over 50 trials, SMOTE handled class imbalance, five-fold cross-validation and a held-out test set were used for evaluation, and SHAP was applied for explainability.

Result: Using 1,641 subjects, the model achieved mean five-fold CV macro AUC 0.983, accuracy 0.944, and macro F1 0.929. On the held-out test set of 247 subjects, it reached macro AUC 0.982 (95% CI 0.965-0.995), accuracy 0.943, balanced accuracy 0.932, macro F1 0.927, and Cohen's kappa 0.909. SHAP showed CDR Global dominated NC/MCI prediction, while CDR-SB and MMSE were key for AD.

Conclusion: An explainable XGBoost model using eight routine clinical assessment features can classify NC, MCI, and AD with near-perfect performance, and SHAP-derived class-specific importance patterns support its clinical validity.
...
TL;DR: TL;DR：这是一项面向机器学习低比特数值计算的浮点标准草案，核心是用可参数化格式、无异常运算语义、丰富舍入模式和形式化验证，来同时提升效率、一致性和可实现性。
TL;DR: This is a draft floating-point standard for low-bit machine-learning computation that combines parameterized formats, exception-free semantics, rich rounding modes, and formal verification to improve efficiency, consistency, and implementability.


<details>
  <summary>Details</summary>
Motivation: 动机是：现有机器学习硬件与软件需要低比特、高吞吐且行为一致的数值格式，但传统浮点标准对这类需求支持不足，且异常处理、近似实现描述和形式化验证能力不够统一。因此需要一个兼顾效率、一致性、可扩展性和可验证性的标准。
The motivation is to address the need in machine learning systems for low-bit, high-throughput, and behaviorally consistent numeric formats, which are not fully served by conventional floating-point standards. A unified standard is needed to support efficient computation, explicit exceptional-value semantics, approximate implementations, and formal verifiability.

Method: 方法上，作者定义了可按位宽、精度、符号性和无穷表示进行参数化的浮点格式；通过将数值解码到闭扩展实数集合来统一定义运算；显式处理NaN和无穷；提供多种舍入与饱和模式（含随机舍入）；定义共享尺度因子的块运算；并使用形式化规范对函数定义与性质进行机械化验证与生成。
Methodologically, the work defines floating-point formats parameterized by width, precision, signedness, and infinity support; specifies operations by decoding values into the closed extended reals; explicitly handles NaN and infinities; includes multiple rounding and saturation modes including stochastic rounding; defines block operations with shared scale factors; and mechanically verifies/generates definitions through formal specifications.

Result: 结果包括：提出IEEE P3109参数化格式族；给出基于闭扩展实数的统一运算语义；提供显式NaN/无穷处理与无异常执行模型；支持丰富的舍入/饱和模式及随机舍入；支持共享尺度块格式；引入kappa-approximation度量近似实现；并完成标准函数和性质的机械化验证/生成。
The main results are the IEEE P3109 parameterized format family, a uniform operation semantics over the closed extended reals, explicit NaN/infinity handling with exception-free execution, rich rounding/saturation modes including stochastic rounding, support for block formats with shared scale factors, the kappa-approximation metric for approximate implementations, and mechanical verification/generation of standard functions and properties.

Conclusion: 该论文/标准草案的结论是：IEEE P3109提出了一套面向机器学习的参数化二进制浮点格式与无异常算术语义，能够在低比特宽度下实现一致、高效、可验证的数值表示与计算，并允许厂商以kappa-approximation形式规范近似实现。
The conclusion is that IEEE P3109 defines a parameterized family of binary floating-point formats and exception-free arithmetic semantics for machine learning, enabling consistent, efficient, and mechanically verifiable low-bit-width numeric representation and computation, while allowing vendors to specify approximate implementations via kappa-approximation.
...
TL;DR: Real-world RL should usually keep learning after deployment, because changing environments make static agents suboptimal; this paper argues for continual RL as the right framing and discusses why and how to adopt it.


<details>
  <summary>Details</summary>
Motivation: Current real-world RL systems are often trained once and only updated after performance degradation, but real environments change over time; the paper is motivated by the mismatch between this static deployment practice and the dynamic nature of real-world interaction.

Method: This is a position paper that conceptually analyzes the deployment setting of RL, identifies four major sources of post-deployment non-stationarity, examines real-world continual RL examples, and argues for a shift from the train-then-fix paradigm to never-ending learning.

Result: The paper reframes many deployed RL problems as continual RL, highlights why top-performing agents must keep adapting after deployment, and summarizes practical advantages and evaluation considerations for moving beyond train-then-fix.

Conclusion: Deployed reinforcement learning agents that continue receiving evaluative rewards should be treated as continual RL systems, because post-deployment non-stationarity makes ongoing adaptation necessary rather than optional.
...
TL;DR: 论文说明：耦合优化的难点不在渐近收敛本身，而在非正规Jacobian导致的巨大瞬态放大；作者用伪谱/Kreiss常数精确量化这种风险，并给出复杂度与稳定性缩放律。 / TL;DR: The main issue in coupled optimization is not just asymptotic convergence, but potentially huge transient amplification caused by non-normal Jacobians; the paper quantifies this sharply via pseudospectra/Kreiss constants and turns it into stability and complexity scaling laws.


<details>
  <summary>Details</summary>
Motivation: 传统仅基于谱半径的渐近稳定性分析无法解释双层优化、双时间尺度随机逼近和对抗训练中常见的强瞬态放大与非平稳学习动力学，因此需要一个能刻画非正规耦合效应的非渐近理论。 / Classical asymptotic stability analysis based only on spectral radius cannot explain the strong transient amplification and non-stationary learning dynamics seen in bilevel optimization, two-time-scale stochastic approximation, and adversarial training, motivating a non-asymptotic theory that captures non-normal coupling effects.

Method: 通过对块三角Jacobian建立尖锐的伪谱理论，推导Kreiss常数上界与匹配的minimax下界；分析耦合强度导致谱不稳定的临界阈值；再用Neumann级数扰动框架扩展到近自指系统，并将这些结果用于随机耦合下降的迭代复杂度分析。 / The method develops a sharp pseudospectral theory for block-triangular Jacobians, deriving upper bounds on the Kreiss constant and matching minimax lower bounds; it characterizes the critical coupling threshold for spectral instability, extends to nearly self-referential systems via a Neumann-series perturbation framework, and applies these results to iteration-complexity analysis of stochastic coupled descent.

Result: 证明当对角块对称且谱半径至多为γ<1时，Kreiss常数满足 K(J) ≤ 2/(1-γ) + ||C||/(4(1-γ))，并给出匹配的minimax下界；建立谱不稳定的临界耦合阈值；对近自指系统给出扩展；推出随机耦合下降的有限时域迭代复杂度 O(K(J)^2 log(1/δ))；实验在线性二次问题、IQC比较和神经网络训练中验证理论。 / The paper proves that when diagonal blocks are symmetric with spectral radii at most γ<1, the Kreiss constant satisfies K(J) ≤ 2/(1-γ) + ||C||/(4(1-γ)), with matching minimax lower bounds; it establishes the critical coupling threshold for spectral instability, extends the analysis to nearly self-referential systems, derives a finite-horizon iteration complexity O(K(J)^2 log(1/δ)) for stochastic coupled descent, and validates the theory on linear-quadratic problems, IQC-based comparisons, and neural-network training.

Conclusion: 耦合梯度下降在块三角Jacobian下，即使对角块谱半径都小于1，非正规性仍可导致很大的瞬态放大；本文给出了对此现象的尖锐伪谱刻画、稳定性阈值与有限时域复杂度界。 / For coupled gradient descent with block-triangular Jacobians, even when diagonal-block spectral radii are below 1, non-normality can induce large transient amplification; the paper provides a sharp pseudospectral characterization, instability thresholds, and finite-horizon complexity bounds.
...
TL;DR: The paper shows that transformers may not need three separate QKV projections: sharing key and value projections often keeps performance nearly intact while greatly shrinking inference memory, especially when combined with GQA or MQA.


<details>
  <summary>Details</summary>
Motivation: Although QKV attention is standard in transformers, the distinct necessity of separate query, key, and value projections is not well understood. The paper aims to determine which projections can be shared without major performance loss and whether doing so yields practical efficiency gains, especially for inference memory and edge deployment.

Method: The paper systematically studies three projection-sharing constraints in attention: Q-K=V (shared key-value), Q=K-V (shared query-key), and Q=K=V (single projection). It evaluates them across synthetic tasks, vision benchmarks, anomaly detection, and large-scale language modeling, and also tests asymmetric attention with 2D positional encodings to mitigate symmetry in some variants. The authors further analyze representation similarity and low-rank behavior to explain the empirical results.

Result: Across tasks, several shared-projection transformers match or occasionally outperform standard QKV transformers. In language modeling, Q-K=V reduces KV cache by 50% with only 3.1% perplexity degradation. Combined with GQA-4 it yields 87.5% cache reduction, and with MQA it reaches 96.9%. The analysis suggests Q-K=V works because keys and values can lie in similar representational spaces and attention often operates in a low-rank regime, while Q=K-V harms directional attention.

Conclusion: Projection sharing in transformer attention is a viable weight-tying strategy. In particular, sharing key and value projections (Q-K=V) preserves most model quality while substantially reducing inference-time KV cache, and it combines effectively with head sharing methods such as GQA and MQA for strong memory savings.
...
TL;DR: 论文用 OpenMC 数据训练可微深度代理模型，并通过梯度优化自动设计临界实验几何，以最大化与目标核系统的 c_k；在 HALEU 运输容器案例中取得了最高 0.97757 的高相似度。 / The paper uses an OpenMC-trained differentiable deep surrogate plus gradient optimization to automatically design critical experiment geometries that maximize c_k with a target nuclear system, reaching up to 0.97757 similarity in a HALEU transport-cask case.


<details>
  <summary>Details</summary>
Motivation: 动机是先进核反应堆与燃料概念的验证需要与目标技术高度中子学相似的临界实验，但现有实验覆盖不足，尤其对 TN-LC 运输容器中的 HALEU 燃料等场景更明显；传统设计方法难以高效搜索满足 c_k≥0.9 的实验。 / The motivation is that validating advanced reactor and fuel concepts requires critical experiments with high neutronic similarity to the target technology, yet existing experimental coverage is limited, especially for cases such as the TN-LC cask with HALEU fuel, and conventional design approaches are inefficient at finding experiments meeting c_k≥0.9.

Method: 方法上，作者先用 OpenMC 计算网格化临界实验几何的灵敏度向量，再训练一个结合 U-Net 编码器-解码器和新型 multigroup attention pooling 的深度神经网络代理模型；随后利用该可微代理在完整组合设计空间中进行非参数梯度优化，直接调整几何网格中每个位置的材料分配，以最大化相关系数 c_k。 / The method trains a differentiable surrogate on OpenMC-generated sensitivity vectors for grid-based critical experiment geometries using a U-Net encoder-decoder plus a novel multigroup attention pooling layer, then performs nonparametric gradient-based optimization over the full combinatorial space by directly changing material assignments at each grid position to maximize c_k.

Result: 结果表明，multigroup attention pooling 优于传统池化并具有可解释性；在 TN-Americas TN-LC 运输容器的三个目标构型上，优化得到的实验几何分别达到 c_k=0.97757、0.81324 和 0.93276，其中两个超过了通常认为足够相似的 0.9 阈值。 / Results show that multigroup attention pooling outperforms traditional pooling and offers interpretable behavior; for three TN-Americas TN-LC cask configurations, the optimized experiments achieved c_k values of 0.97757, 0.81324, and 0.93276, with two exceeding the commonly used 0.9 similarity threshold.

Conclusion: 该论文提出了一种用于临界实验逆向设计的方法，利用深度神经网络代理模型与梯度优化，能够自动生成与目标先进反应堆/燃料技术具有高中子学相似性的实验几何，并显示出加速先进核技术验证的潜力。 / The paper concludes that inverse design of critical experiments using a deep neural surrogate and gradient optimization can automatically generate experiment geometries with high neutronic similarity to target advanced reactor/fuel systems, showing strong potential to accelerate validation workflows.
...
### [191] [EvalStop: Using World Feedback to Detect and Correct Reward Overoptimization in Multi-Tenant RLHF Platforms](https://arxiv.org/abs/2606.04145)
...
### [192] [RUBAS: Rubric-Based Reinforcement Learning for Agent Safety](https://arxiv.org/abs/2606.04051)
...
### [193] [A Goal-Set Characterization of Task Composition in the Boolean Task Algebra](https://arxiv.org/abs/2606.04053)
...
### [194] [Spectral Scaling Laws of Muon](https://arxiv.org/abs/2606.04058)
...
### [196] [TPA-AD: A Two-Stage Pseudo Anomaly-Guided Method for Bearing Time-Series Anomaly Detection](https://arxiv.org/abs/2606.04073)
...
### [197] [Adaptive Patching Is Harder Than It Looks For Time-Series Forecasting](https://arxiv.org/abs/2606.04074)
...
### [198] [Large Language Models Hack Rewards, and Society](https://arxiv.org/abs/2606.04075)
...
### [199] [Stein Kernelized Molecular Dynamics for Active Learning of Interatomic Potentials](https://arxiv.org/abs/2606.04100)
...
### [200] [Building The Ph(ysical)AI Layer Of Machine Intelligence](https://arxiv.org/abs/2606.04106)
...
### [201] [Variance Reduction for Heavy-Tailed Monetization Metrics in Ranking Experiments via Post-Stratification](https://arxiv.org/abs/2606.04110)
...
### [202] [dMX: Differentiable Mixed-Precision Assignment for Low-Precision Floating-Point Formats](https://arxiv.org/abs/2606.04115)
...
### [203] [Stationarity-Aware Retrieval-Augmented Time Series Forecasting](https://arxiv.org/abs/2606.04135)
...
### [204] [Physics-Informed Machine Learning for Short-Term Flood Prediction](https://arxiv.org/abs/2606.04143)
...
### [283] [Reproducing, Analyzing, and Detecting Reward Hacking in Rubric-Based Reinforcement Learning](https://arxiv.org/abs/2606.04923)
...
### [284] [Uncertainty-Aware (Un)Supervised Few-Shot User Adaptation for On-Device Personalized Human Activity Recognition](https://arxiv.org/abs/2606.04798)
...
### [285] [Data Attribution in Large Language Models via Bidirectional Gradient Optimization](https://arxiv.org/abs/2606.04928)
...
### [288] [Validity Threats for Foundation Model Research](https://arxiv.org/abs/2606.05029)
...
### [289] [Learning While Acting: A Skill-Enhanced Test-Time Co-Evolution Framework for Online Lifelong Learning Agents](https://arxiv.org/abs/2606.04815)
...
### [290] [In-Context Graphical Inference](https://arxiv.org/abs/2606.05042)
...
### [293] [Toward Multi-Domain and Long-Tailed Quantization via Feature Alignment and Scaling](https://arxiv.org/abs/2606.04920)
...
### [294] [Failed Reasoning Traces Tell You What Is Fixable (But Not by Reading Them)](https://arxiv.org/abs/2606.05145)
...
### [296] [Identifying Gems from Roman RAPIDly](https://arxiv.org/abs/2606.05103)
...
### [297] [Reinforcement Learning from Rich Feedback with Distributional DAgger](https://arxiv.org/abs/2606.05152)
...
### [299] [STRIDE: Training Data Attribution via Sparse Recovery from Subset Perturbations](https://arxiv.org/abs/2606.05165)
...
### [324] [Polymarket-v1 Database](https://arxiv.org/abs/2606.04217)
*Boka Qin,Rui Yang*

Main category: cs.CE

TL;DR: A massive on-chain Polymarket dataset with true trade direction shows that classical trade-sign classifiers fail in prediction markets, distort key microstructure metrics, and obscure real links between market quality and forecasting accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing prediction market datasets infer trade direction heuristically, which can distort microstructure measurement and downstream conclusions. The authors aim to provide a complete ground-truth archive and test whether classical market microstructure tools remain reliable in prediction markets.

Method: The paper constructs the Polymarket-v1 Database, a complete on-chain archive of 1.20 billion trades across 1.30 million markets, with aggressor direction directly observed from settlement data. Using this truth-aligned dataset, it benchmarks standard trade classification methods such as the tick rule and bulk volume classification, and compares downstream microstructure metrics including VPIN, OFI, and Gibbs spread against forecasting outcomes measured by Brier scores.

Result: Standard classifiers perform near random in aggregate accuracy (49.83% for tick rule, 50.51% for bulk volume classification), with systematic price-level bias driven by trade direction autocorrelation and concentrated market-making. These misclassifications materially distort inferred VPIN and OFI. In contrast, ground-truth metrics show that True VPIN positively predicts Brier scores while Gibbs spread negatively predicts them; these relationships are attenuated when proxy classifications are used.

Conclusion: Ground-truth trade direction from blockchain settlement is essential for valid microstructure analysis in prediction markets; heuristic trade classification produces large errors and weakens inference about market quality and forecasting performance.
...
### [325] [When Does Structure Help? The Information Bonus of AlphaFold2 Representations over Protein Language Models](https://arxiv.org/abs/2606.04228)
*Kargi Chauhan*

Main category: cs.CE

TL;DR: The paper proposes Information Bonus (IB) to measure whether expensive structural protein representations are worth using over cheaper sequence-only embeddings. Sequence-only ESM-2 works better for binding affinity and flexibility, but AlphaFold2 structural representations add unique value for allostery. It also warns that residue-level data leakage can seriously mislead benchmarking.


<details>
  <summary>Details</summary>
Motivation: AI-driven protein pipelines often have to decide whether the extra computational cost of structure-based models is justified relative to cheaper sequence-only models. The paper aims to turn this choice into a measurable engineering and scientific decision by evaluating when structural inference adds useful predictive information.

Method: The paper defines Information Bonus (IB), a task-level metric quantifying the linearly accessible advantage of frozen single-sequence AlphaFold2 Evoformer representations over frozen ESM-2 embeddings. The authors compare the two frozen representations under protein-level cross-validation on three tasks: binding affinity regression (PDBbind), conformational flexibility prediction (ATLAS molecular dynamics), and allosteric-site classification (AlloSigDB). They also test the effect of naive residue-level data splitting to identify leakage artifacts.

Result: Results are strongly task-dependent. ESM-2 outperforms AlphaFold2 representations on binding affinity regression (IB = -0.141; Pearson r = 0.449 vs. 0.307) and binary flexibility prediction (IB = -0.060; AUROC 0.824 vs. 0.764; p = 0.0017). In contrast, AlphaFold2 representations are the only ones achieving above-chance allosteric-site prediction (IB = +0.064; AUROC 0.548 vs. 0.485), suggesting recovery of long-range geometric information unavailable from sequence alone. The study also finds residue-level leakage: naive residue splits inflate RMSF performance by 27–39%, enough to reverse representation rankings.

Conclusion: Structural inference is not universally better in protein ML; its value depends on the biological mechanism and task. Sequence-only ESM-2 is better for binding affinity and flexibility, while AlphaFold2 Evoformer structural representations provide unique signal for allosteric-site prediction. Proper protein-level splitting is essential to avoid residue-level leakage artifacts that can distort model comparisons.
...
### [326] [UltraEP: Unleash MoE Training and Inference on Rack-Scale Nodes with Near-Optimal Load Balancing](https://arxiv.org/abs/2606.04101)
...
TL;DR: UltraEP uses per-microbatch, per-layer exact load balancing for MoE expert parallelism on rack-scale nodes, substantially improving throughput and reducing imbalance versus prior non-real-time approaches.


<details>
  <summary>Details</summary>
Motivation: Existing expert balancers rely on periodic redistribution using historical load, which is inadequate under non-stationary production workloads and leads to stragglers, all-to-all bottlenecks, and activation-memory spikes in large-scale expert parallelism.

Method: The paper proposes UltraEP, which rebalances experts at every microbatch and layer using quota-driven planning based on post-gating load, and performs irregular expert-state transfers with persistent tile streaming and relay-based fan-out mitigation on rack-scale node connectivity.

Result: Across 106B to 671B MoE models for training and prefill, UltraEP reaches 94.3% of force-balanced ideal throughput, improves throughput by 1.49x over no balancing, reduces inter-rank imbalance from 1.30-4.01 to 1.01-1.04, and scales robustly to 2560 GPUs in production training.

Conclusion: UltraEP is an exact-load, real-time balancer for large-scale expert parallel MoE training and prefill on rack-scale nodes, achieving near-ideal throughput and strong production scalability.
...
### [327] [ACEAPEX: Parallel LZ77 Decoding via Encode-Time Absolute Offset Resolution](https://arxiv.org/abs/2606.04268)
*Yakiv Shavidze*

Main category: cs.DC

TL;DR: ACEAPEX redesigns LZ77 references to make decoding massively parallel. It delivers much faster CPU and GPU decompression than standard LZ77 implementations, while preserving near-standard compression ratios and bit-exact correctness.


<details>
  <summary>Details</summary>
Motivation: Conventional LZ77 decoders are fundamentally sequential because each back-reference depends on previously reconstructed data, which limits multi-core and GPU acceleration. The work aims to break this bottleneck without sacrificing much compression efficiency.

Method: The paper introduces ACEAPEX, a parallel LZ77 codec that converts back-references into absolute positions in the decompressed output and packages data into self-contained 1 MB blocks for independent decoding. It also implements a GPU wavefront decoder and a depth-limited encoder variant to further improve GPU throughput.

Result: Integrated into lzbench, ACEAPEX reaches 10,160 MB/s on EPYC 4344P and 10,869 MB/s on EPYC 9575F for FASTQ data, up to 3.1x faster than zstd -3 at similar compression ratio. On NVIDIA H100 SXM, the GPU decoder achieves 44.0 GB/s on enwik9 and 20.3 GB/s on FASTQ, and with the depth-limited encoder reaches 77.2 GB/s on one H100 and 249.9 GB/s on two H100s via NVLink, with bit-perfect verification.

Conclusion: ACEAPEX removes the main decode-side sequential dependency of LZ77 at the block level, enabling strong CPU/GPU parallel scaling while keeping compression ratio close to standard LZ77 codecs.
...
### [328] [FlexNPU: Transparent NPU Virtualization for Dynamic LLM Prefill-Decode Co-location](https://arxiv.org/abs/2606.04415)
...
TL;DR: FlexNPU is a transparent virtualization layer for Ascend NPUs that lets the runtime schedule LLM prefill and decode more intelligently. It preserves baseline performance, improves throughput versus static PD disaggregation, and dramatically cuts TTFT versus static co-location.


<details>
  <summary>Details</summary>
Motivation: Existing NPU deployments expose physical devices directly to applications, limiting runtime scheduling control and making it hard to adapt to phase-level workload behavior. In LLM serving, prefill is compute-intensive while decode is memory/KV-cache bound, so static prefill-decode disaggregation or co-location often causes imbalance, interference, or unnecessary data movement.

Method: The paper designs FlexNPU as a user-space virtualization layer that interposes on AscendCL APIs and forwards NPU operations to per-device daemons. This decouples applications from physical NPUs, virtualizes NPU objects, controls operator dispatch, and enables phase-aware scheduling, especially dynamic co-location of prefill and decode phases in LLM serving.

Result: FlexNPU adds no measurable inference overhead compared with direct NPU passthrough and slightly improves throughput in some cases. On a 384-card Ascend 910C deployment of DeepSeek-R1, it improves throughput over static PD disaggregation by 5.15% and 26.33%. On Qwen2.5-7B, it achieves comparable throughput to static PD co-location while reducing TTFT by over 92% with nearly unchanged TPOT.

Conclusion: FlexNPU demonstrates that transparent user-space virtualization for Ascend NPUs can support efficient, phase-aware LLM serving. It enables dynamic prefill-decode co-location and improves throughput or latency compared with static deployment strategies, without measurable inference overhead versus direct passthrough.
...
### [329] [D^2SD: Accelerating Speculative Decoding with Dual Diffusion Draft Models](https://arxiv.org/abs/2606.04446)
*Liyuan Zhang,Jiarui Zhang,Jinwei Yao,Ran Yan,Yuchen Yang,Jiahao Zhang,Tongkai Yang,Yi Wu,Binhang Yuan*

Main category: cs.DC

TL;DR: D^2SD在标准扩散草稿器后面加了一个“恢复式”第二草稿器：先用置信度找出最可能出错的位置，再只对有价值的前缀分支补充候选，并把这些共享前缀候选一起高效验证，因此比单一路径或盲目多分支更快、更能保留可接受 token。 / D^2SD adds a recovery-oriented second drafter after a standard diffusion drafter: it uses confidence to locate likely failure points, expands only valuable prefix branches, and verifies those shared-prefix candidates efficiently together, making it faster and more acceptance-efficient than single-path or naive multi-branch drafting.


<details>
  <summary>Details</summary>
Motivation: 现有扩散式 speculative decoding 虽能并行生成整段草稿，但通常每次验证只依赖单一路径；一旦首个不匹配出现，后续 token 全部作废，接受率受限。简单增加候选序列数又会带来冗余分支和额外验证成本，收益有限。 / Existing diffusion-based speculative decoding can draft full blocks in parallel, but usually verifies only a single path; once the first mismatch appears, all later drafted tokens are discarded, limiting acceptance. Naively adding more candidate sequences causes redundant branches and extra verification cost with limited gain.

Method: 方法包括两阶段起草与联合验证：第一扩散起草器并行生成一个 token block 及逐位置置信度；基于这些分数估计最可能的拒绝边界，并选择 top-K 前缀区间；第二个可变前缀扩散起草器分别从这些前缀重新锚定，在一次批处理中生成替代续写；最后利用 cascade attention 对共享前缀候选进行联合验证。 / The method uses two-stage drafting and joint verification: the first diffusion drafter generates a token block plus per-position confidence scores; these scores estimate the likely rejection boundary and select top-K prefix ranges; a second variable-prefix diffusion drafter re-anchors at each prefix to generate alternative continuations in one batched pass; finally, shared-prefix candidates are jointly verified with cascade attention.

Result: 实验表明，D^2SD相较于其底层扩散式方法以及强自回归 speculative decoding 基线均有明显提升。摘要未给出具体数值，但结论是其在推理加速/接受效率方面具有稳定优势。 / Empirically, D^2SD shows clear gains over its underlying diffusion approach and strong autoregressive speculative decoding baselines. The abstract does not report exact numbers, but the claimed outcome is a consistent advantage in inference acceleration and token acceptance efficiency.

Conclusion: D^2SD通过双扩散起草器与置信度引导的前缀树恢复机制，提高了 speculative decoding 中已起草 token 的接受率，并在效率上优于现有扩散式和强自回归基线。 / D^2SD improves speculative decoding by using dual diffusion drafters and a confidence-guided prefix-tree recovery mechanism, increasing accepted drafted tokens and outperforming prior diffusion and strong autoregressive baselines.
...
### [330] [Multi-SPIN: Multi-Access Speculative Inference for Cooperative Token Generation at the Edge](https://arxiv.org/abs/2606.04581)
*Haotian Zheng,Zhanwei Wang,Mingyao Cui,Chang Cai,Hongyang Du,Kaibin Huang*

Main category: cs.DC

TL;DR: The paper proposes Multi-SPIN, a distributed speculative decoding framework for multiuser edge systems, and shows that jointly optimizing token draft lengths and bandwidth allocation can substantially improve token generation throughput, achieving up to 88% gain over baseline methods.


<details>
  <summary>Details</summary>
Motivation: Single-user speculative inference does not directly address multiuser edge environments with heterogeneous computation and communication resources. The paper is motivated by the need to balance device/server workloads and reduce multi-access latency so that overall cooperative token generation efficiency can be improved.

Method: The paper extends speculative inference to a multi-access edge setting where on-device small language models draft tokens and an edge LLM verifies them in parallel. It formulates a joint optimization of draft-length control and FDMA bandwidth allocation for maximizing sum token goodput, considering both homogeneous and heterogeneous draft-length cases, and derives closed-form efficient algorithms via problem decomposition.

Result: The analysis characterizes different optimal bandwidth allocation behaviors in homogeneous versus heterogeneous draft settings: homogeneous batching favors weaker users to reduce synchronization bottlenecks, while heterogeneous drafting favors users with higher token acceptance rates. Experiments with Llama-2 and Qwen3.5 across tasks show up to 88% higher goodput than heterogeneity-agnostic baselines.

Conclusion: Multi-SPIN enables distributed speculative inference in multiuser edge systems and, with proper joint draft-length and bandwidth control, significantly improves system token goodput under user heterogeneity.
...
### [331] [Ekka: Automated Diagnosis of Silent Errors in LLM Inference](https://arxiv.org/abs/2606.04594)
*Yile Gu,Zhen Zhang,Shaowei Zhu,Xinwei Fu,Jun Wu,Yida Wang,Baris Kasikci*

Main category: cs.DC

TL;DR: Ekka diagnoses silent quality regressions in LLM serving frameworks by differentially comparing internal states with a correct reference implementation, reaching 80% pass@1 and 88% pass@5 accuracy and finding 4 new confirmed bugs.


<details>
  <summary>Details</summary>
Motivation: LLM serving frameworks evolve rapidly and include complex software stacks with many optimizations, which can introduce silent errors that degrade output quality without explicit failure signals. These errors are difficult to diagnose because symptoms appear at a semantic level while causes lie deep in low-level execution details.

Method: The paper proposes Ekka, an automated diagnosis system that compares a target LLM serving framework against a semantically correct reference framework. It systematically aligns and contrasts intermediate execution states to trace high-level output-quality degradation back to low-level root causes.

Result: On a benchmark of real-world silent errors from popular serving frameworks, Ekka achieves 80% pass@1 diagnosis accuracy and 88% pass@5 diagnosis accuracy, outperforming state-of-the-art systems. It also identified 4 new silent errors, all confirmed by framework developers.

Conclusion: Ekka can effectively diagnose silent errors in LLM serving frameworks by framing diagnosis as differential debugging and aligning intermediate execution states with a correct reference implementation. It achieves strong benchmark performance and can uncover previously unknown real-world silent errors.
...
### [332] [Rectangular Matrix Multiplication in the Low-Bandwidth Model](https://arxiv.org/abs/2606.04652)
*Chetan Gupta,Jukka Suomela,Hossein Vahidi*

Main category: cs.DC

TL;DR: 论文研究低带宽分布式环境下的稠密矩形矩阵乘法，证明矩阵形状会显著改变通信轮复杂度：<d,n,d> 延续了方阵的 4/3 指数，而 <n,d,n> 在 d=√n 附近发生相变，小 d 时为 Θ(d√n)，较大 d 时改为 O(d^{2/3}n^{2/3})。 / The paper studies dense rectangular matrix multiplication in the low-bandwidth distributed model and shows that matrix shape strongly changes round complexity: <d,n,d> preserves the square-case 4/3-type behavior, while <n,d,n> undergoes a phase transition near d=√n, with Θ(d√n) for small d and O(d^{2/3}n^{2/3}) for larger d.


<details>
  <summary>Details</summary>
Motivation: 以往工作主要研究方阵乘法或依赖稀疏性假设，但实际分布式计算中矩形且稠密的矩阵乘法同样自然且重要。作者希望理解矩阵长宽比如何影响通信复杂度，以及方阵结果能否自然推广到矩形情形。 / Prior work mainly addressed square matrix multiplication or relied on sparsity assumptions, while dense rectangular multiplication is also natural and important in distributed settings. The motivation is to understand how aspect ratio affects communication complexity and whether square-matrix bounds generalize to rectangular cases.

Method: 作者研究低带宽分布式模型中矩阵乘法的通信轮复杂度，按矩阵形状参数化任务 <a,b,c>，重点分析 <n,d,n> 和 <d,n,d>。方法上通过构造针对不同长宽比的分布式算法获得上界，并通过匹配或接近匹配的下界分析刻画其最优性与相变区域。 / The authors study communication-round complexity in the low-bandwidth distributed model, parameterizing matrix multiplication as <a,b,c> and focusing on <n,d,n> and <d,n,d>. Methodologically, they design distributed algorithms tailored to these aspect ratios to derive upper bounds, and complement them with matching or near-matching lower bounds to characterize optimality and phase transitions.

Result: 主要结果包括：1) 对 <d,n,d>，得到 ẼO(d^{4/3}) 轮复杂度；2) 对 <n,d,n>，当 d≤√n 时复杂度为 Θ(d√n)，且上下界匹配；3) 当 d≥√n 时，上界为 O(d^{2/3}n^{2/3})。这些结果显示两类矩形乘法具有不同的复杂度规律。 / Main results: (1) for <d,n,d>, the round complexity is ẼO(d^{4/3}); (2) for <n,d,n>, when d≤√n, the complexity is Θ(d√n) with matching upper and lower bounds; (3) when d≥√n, the upper bound is O(d^{2/3}n^{2/3}). These results reveal distinct complexity laws for the two rectangular settings.

Conclusion: 该论文给出了低带宽分布式计算模型下两类矩形矩阵乘法任务的轮复杂度上界与部分紧下界，表明 <d,n,d> 与 <n,d,n> 在复杂度上存在本质差异，并证明 <n,d,n> 在 d≈√n 附近出现相变。 / The paper establishes round-complexity bounds for two rectangular matrix multiplication tasks in the low-bandwidth distributed model, showing a fundamental separation between <d,n,d> and <n,d,n>, and proving a phase transition for <n,d,n> around d≈√n.
...
### [333] [Clownfish: Scaling DAG-based BFT Consensus via Sparse Edges](https://arxiv.org/abs/2606.04687)
...
### [334] [The local complexity of certifying parity](https://arxiv.org/abs/2606.04934)
...
### [335] [Graph Traversal on Tensor Cores: A BFS Framework for Modern GPUs](https://arxiv.org/abs/2606.05081)
...
### [336] [The relationship between atmospheric stratification and internal wave processes](https://arxiv.org/abs/2606.04590)
*A. V. Kochin*

Main category: physics.ao-ph

TL;DR: By treating the atmosphere as a resonant system, the paper uses surface pressure fluctuation spectra and radiosonde reference data to identify internal gravity waves and estimate stratification parameters, achieving results consistent with upper-air soundings.


<details>
  <summary>Details</summary>
Motivation: Because the atmosphere behaves as a resonant system whose oscillation spectrum depends on spatial parameter distributions, spectral analysis of internal wave processes may provide a way to retrieve atmospheric structure such as stratification.

Method: The study detects atmospheric wave fluctuations by analyzing spectra of surface pressure variations and compares them with radiosonde ascent-rate reference data. It then estimates stratification parameters from the frequencies of internal gravity waves.

Result: Internal gravity wave frequencies derived from measured spectra were successfully used to calculate atmospheric stratification parameters, with good agreement against radiosonde sounding data.

Conclusion: The measured surface pressure fluctuation spectra can be used to infer atmospheric stratification parameters, and the inferred values agree well with upper-air sounding observations.
...
### [337] [SymTRELLIS: Symmetry-Enforced Voxel Latents for 3D Generation](https://arxiv.org/abs/2606.04108)
*Guangda Ji,Qimin Chen,Qinchan Li,Mingrui Zhao,Kai Wang,Hao Zhang*

Main category: cs.GR

TL;DR: SymTRELLIS adds symmetry control to TRELLIS.2 without retraining: it learns how latent features transform under symmetry operations and symmetrizes flow velocities during sampling, yielding much more symmetric 3D objects with little loss in fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing single-view 3D generative models produce visually strong results but often fail structural and functional constraints. Symmetry violations, even small ones, can make generated objects physically unusable, so a way to enforce symmetry during generation is needed.

Method: The method learns a lightweight linear latent-space mapper that approximates how spatial transformations act on voxel latents. During ODE-based flow generation, it enforces symmetry by averaging predicted flow velocities over all symmetry-equivalent transformed states, called velocity symmetrization. Symmetry can be auto-estimated from an initial generation or specified by the user.

Result: On a benchmark of 266 strictly symmetric objects covering rotational and polyhedral symmetry groups, SymTRELLIS outperforms TRELLIS.2, Hunyuan3D-2.1, and TripoSG on all symmetry error metrics, while keeping reconstruction accuracy comparable to the original model.

Conclusion: SymTRELLIS can impose arbitrary finite point-group symmetries during 3D generation without retraining the base TRELLIS.2 models, significantly reducing symmetry errors while preserving reconstruction quality.
...
### [338] [PureLight: Learning Complex Luminaires with Light Tracing](https://arxiv.org/abs/2606.04319)
*Pedro Figueiredo,Zixuan Li,Beibei Wang,Miloš Hašan,Nima Khademi Kalantari*

Main category: cs.GR

TL;DR: A neural renderer learns how complicated lamps emit light by tracing light paths, modeling exit-surface radiance with a normalizing flow, then compressing it into a small network for fast rendering and better low-sample performance.


<details>
  <summary>Details</summary>
Motivation: Complex luminaires with difficult internal light transport, such as small emitters behind multiple specular layers, are hard to render efficiently with traditional bidirectional path tracing. The work is motivated by the need for accurate appearance estimation and practical low-sample rendering for such cases.

Method: The method uses light tracing to build paths from emitters to exit surfaces, learns the PDF of outgoing radiance on exit surfaces with a large normalizing flow, recovers radiance as PDF times flux, distills the result into a lightweight MLP for fast inference, and trains additional sampling and blending networks for direct illumination and scene compositing.

Result: The proposed approach enables efficient rendering of challenging luminaires at low sample counts while supporting arbitrary scenes, by combining accurate learned appearance estimation with efficient inference and sampling.

Conclusion: The paper concludes that a neural, distribution-learning-based formulation can accurately and efficiently model the appearance of complex luminaires, making low-sample rendering feasible in arbitrary scenes.
...
### [339] [Geometry Gaussians: Decoupling Appearance and Geometry in Gaussian Splatting](https://arxiv.org/abs/2606.05124)
*Hongyu Zhou,Zorah Lähner*

Main category: cs.GR

TL;DR: Standard 3DGS cannot reliably represent texture and geometry with the same opacity variable; adding one extra geometry opacity per splat is a simple fix that improves both surface reconstruction and rendering, especially for transparent scenes.


<details>
  <summary>Details</summary>
Motivation: Prior work tried to extract geometric surfaces from 3DGS, but direct geometry recovery from standard 3DGS is difficult and can degrade rendering quality. The authors are motivated by the need to improve geometric representation without sacrificing the strong novel-view rendering performance of 3DGS.

Method: The paper first diagnoses a structural limitation of standard 3D Gaussian Splatting by training it with full ground-truth texture and geometry supervision, showing that one shared opacity representation is not sufficient for jointly modeling appearance and surface geometry. It then introduces a minimal modification: an extra geometry opacity parameter per splat, plus an optional optimization pipeline designed to better handle transparency.

Result: Experiments using both ground-truth geometric supervision and geometry derived from vision foundation models show consistent gains in rendering and geometry metrics across diverse datasets. The improvements are particularly strong for challenging scenes with transparent objects.

Conclusion: Adding a single geometry-specific opacity parameter to each 3D Gaussian splat, optionally combined with a transparency-curated optimization pipeline, improves both geometric accuracy and rendering quality compared with default 3DGS, especially in complex scenes containing transparent objects.
...
### [340] [Toward Pre-Deployment Assurance for Enterprise AI Agents: Ontology-Grounded Simulation and Trust Certification](https://arxiv.org/abs/2606.04037)
*Thanh Luong Tuan,Abhijit Sanyal*

Main category: cs.AI

TL;DR: The paper introduces an ontology-based framework to verify enterprise AI agents before deployment. It automatically generates compliance and adversarial test scenarios from an ontology and issues a trust certificate. In regulated domains, it improves regulatory coverage and domain specificity over a persona baseline, but some gains weaken under stricter statistical correction.


<details>
  <summary>Details</summary>
Motivation: Existing safeguards for enterprise AI agents, such as post-deployment monitoring, human oversight, and prompt guardrails, do not provide strong assurance once an agent is already in production. There is a gap between generic LLM capability benchmarking and pre-deployment verification for real enterprise deployment, especially in regulatory-intensive domains.

Method: The paper proposes a three-part framework: (1) an Agent Operational Envelope to formalize certification dimensions such as permissions, constraints, safety, governance, and autonomy; (2) an ontology-to-scenario generation pipeline that automatically derives regulatory, operational, and adversarial test scenarios; and (3) a machine-verifiable Trust Certificate with deployment verdicts. The framework was evaluated in a controlled pilot across regulated industries and regulatory regimes, with comparisons against persona-based and retrieval-augmented baselines, plus cross-validation on multiple LLM families.

Result: In five industry-regulatory cells spanning Fintech, Banking, Insurance, and Healthcare in the US and Vietnam, the framework generated 1,800 scenarios evaluated against 125 regulatory requirements and 25 injected faults. The ontology-grounded approach achieved 48.3% regulatory coverage versus 33.1% for the persona baseline, with corrected p = .0006, and the highest domain specificity at 4.77/5.0 with p = 2e-6. However, its coverage advantage over persona and retrieval-augmented prompting was not robust after Bonferroni correction. Cross-validation over three LLM families and 5,400 scenarios reproduced the ontology-versus-persona trend.

Conclusion: An ontology-grounded pre-deployment verification framework can meaningfully improve regulatory coverage and domain specificity for enterprise AI agent testing in regulated settings, and serves as a credible complement to persona-based test suites, though some advantages are not robust under stricter multiple-comparison correction.
...
### [341] [Stumbling Into AI Emotional Dependence: How Routine AI Interactions Reshape Human Connection](https://arxiv.org/abs/2606.04150)
*Yaoxi Shi,Cathy Mengying Fang,Pattie Maez,Amit Goldenberg*

Main category: cs.AI

TL;DR: TL;DR: AI emotional support is often accidental, not intentional. Once people have positive emotional experiences with general-purpose AI, they become more likely to seek support from AI again and less likely to seek support from humans. Policy should therefore regulate not only companion bots but also general-purpose AI and its long-term effects on human relationships.


<details>
  <summary>Details</summary>
Motivation: 作者的动机是纠正公共讨论与新兴政策中的一个核心误判：人们通常假设AI情感支持是用户有意识、专门寻求的行为，且主要发生在陪伴型应用中。论文认为，这一图景忽略了AI情感支持在通用平台中偶发产生的现实，以及这种经历对人际连接的长期影响。

Method: 本文主要采用综述与论证的方法，整合新近实证证据，并引用一项与OpenAI合作开展的大规模纵向研究。该研究让参与者在28天内每天与AI进行5分钟关于个人问题的对话，以观察其对未来情感支持偏好的变化。

Result: 论文总结的关键结果是：AI情感支持常在任务型、通用型交互中偶然出现；这类体验具有路径依赖性，会更新人们对AI情感能力的认知，并改变未来求助选择。文中引用的纵向研究显示，连续28天每日5分钟与AI讨论个人问题后，参与者寻求人类支持的偏好下降了10.3%，对AI的偏好上升了11.6%。

Conclusion: 当前关于AI情感支持的政策框架过于狭窄，因为AI情感支持往往并非来自用户有意使用陪伴型聊天机器人，而是会在通用、任务导向的AI互动中偶然出现，并在时间上累积地改变用户未来寻求情感支持的对象。因此，监管应覆盖通用型AI系统，并关注人类支持偏好被逐步重定向的轨迹性风险。
...
### [342] [Thinking Through Signs: PEEL as a Semiotic Scaffolding for Epistemically Accountable AI-Enabled Research](https://arxiv.org/abs/2606.04152)
*Clarisse de Souza,Gabriel Barbosa,Simone Diniz Junqueira Barbosa,Bárbara Betts,Renato Cerqueira,Juliana Jansen Ferreira*

Main category: cs.AI

TL;DR: PEEL is a research scaffold that pairs Voyant Tools with Claude to audit AI-generated summaries. It shows that LLM outputs can be fluent yet epistemically distorted, so deterministic measurement should accompany AI use in research.


<details>
  <summary>Details</summary>
Motivation: The motivation is that large language models are transforming research practice while subtly undermining researchers' epistemic accountability, especially because fluent AI outputs can conceal distortions that are hard to detect through impressionistic reading alone.

Method: The paper proposes PEEL (Protocols for Epistemically Engaged Literacy in AI), a scaffold combining deterministic distant reading through Voyant Tools with LLM-based interpretation through Claude, theoretically grounded in Peircean semiotics and abductive reasoning. It applies this framework to AI-generated condensations of three source texts and compares measurable textual features such as quantity, term frequency, and epistemic voice.

Result: Using PEEL, the authors identify systematic distortions in AI-generated condensations, including changes in quantity, shifts in term frequency, and alterations in epistemic voice that would remain invisible without non-AI measurement.

Conclusion: PEEL demonstrates that relying on LLM-generated interpretations without deterministic measurement weakens epistemic accountability; therefore, AI-supported research should pair LLMs with deterministic tools and explicitly design for epistemic authority.
...
### [343] [Plan First, Judge Later, Run Better: A DMAIC-Inspired Agentic System for Industrial Anomaly Detection](https://arxiv.org/abs/2606.04599)
*Yongzi Yu,Ao Li,Le Wang,Ziyue Li,Fugee Tsung,Yuxuan Liang,Man Li*

Main category: cs.AI

TL;DR: DMAIC-IAD applies the DMAIC quality-management idea to LLM agents for industrial anomaly detection: standardize knowledge into SOPs, plan strategies first, and rank them with an execution-free judge. This yields more reliable, unified, and cheaper anomaly detection, with a 37.76% average gain over prior agentic baselines.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based industrial anomaly detection agents focus too much on execution and insufficiently on strategy formulation. This limits their ability to handle heterogeneous data modalities in a unified, reliable, and cost-effective way for high-stakes industrial applications.

Method: The paper proposes DMAIC-IAD, a DMAIC-inspired multi-agent system following a 'Plan First, Judge Later' paradigm. It first distills heterogeneous references into standardized operating procedures (SOPs), then generates multiple candidate detection strategies, and finally uses a pre-trained execution-free judge model to rank strategies without expensive runtime trials.

Result: Across four modalities, DMAIC-IAD achieves an average detection performance improvement of 37.76% over applicable agentic baselines.

Conclusion: DMAIC-IAD is an effective and cost-efficient multi-agent framework for industrial anomaly detection. By planning before execution and using an execution-free judge model, it improves reliability and performance across heterogeneous modalities in industrial settings.
...
### [344] [SMAC-Talk: A Natural Language Extension of the StarCraft Multi-Agent Challenge for Large Language Models](https://arxiv.org/abs/2606.04202)
*Joel Sol,Homayoun Najjaran*

Main category: cs.AI

TL;DR: SMAC-Talk adds natural-language communication to SMAC so researchers can test whether LLM agents can coordinate, trust, and resist deception in cooperative StarCraft-style tasks.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly deployed alongside other AI agents, there is a need for realistic benchmarks that test communication, information sharing, uncertainty handling, coordination, and trust in cooperative multi-agent settings.

Method: The paper extends the StarCraft Multi-Agent Challenge with a natural-language communication channel, creating evaluation settings with decentralized control, partial observability, long-horizon tasks, and deceptive communicators. It benchmarks three agents built with four Qwen3.5-family models to test effects of reasoning structure, memory, and model scale.

Result: The work provides SMAC-Talk and a benchmarking study showing how reasoning structure, memory, and model scale influence coordination performance among LLM-based agents in cooperative environments.

Conclusion: SMAC-Talk is introduced as an open benchmark for studying how LLM agents coordinate, communicate, and maintain trust in cooperative multi-agent environments, including under deceptive communication.
...
### [345] [Consensus is Strategically Insufficient: Reasoning-Trace Disagreement as a Knowledge-Representation Signal](https://arxiv.org/abs/2606.04223)
*Michał Wawer,Jarosław A. Chudziak*

Main category: cs.AI

TL;DR: Instead of making AI agents always agree, the paper says we should classify why they disagree and use that information to route difficult value-sensitive cases more intelligently.


<details>
  <summary>Details</summary>
Motivation: The motivation is that standard multi-agent coordination methods such as voting, consensus, and aggregation often treat disagreement as a problem to eliminate. For value-laden domains like content moderation, however, disagreement can encode meaningful uncertainty and should be modeled rather than suppressed.

Method: The method introduces a symbolic knowledge-representation layer that abstracts agents’ reasoning traces and binary decisions into four disagreement states: convergent agreement, divergent agreement, convergent disagreement, and divergent disagreement. These states are then used with defeasible routing rules to guide downstream handling in a multi-agent system.

Result: The paper instantiates the framework in content moderation and argues that disagreement-aware routing can connect sub-symbolic LLM deliberation with symbolic strategic reasoning. Its main result is a conceptual framework for classifying disagreement and using that classification for strategic decision routing.

Conclusion: The paper concludes that forcing multi-agent systems toward consensus is inadequate for value-laden tasks. Instead, disagreement should be explicitly represented and strategically routed, because some disagreements reflect legitimate normative uncertainty rather than simple error.
...
### [346] [VAMPS: Visual-Assisted Mathematical Problem Solving Benchmark](https://arxiv.org/abs/2606.04244)
...
TL;DR: VAMPS is a bilingual benchmark for testing whether multimodal LLMs can solve graph-friendly math problems by generating and using plots; current models usually do better solving directly than solving through tool-generated visualizations.


<details>
  <summary>Details</summary>
Motivation: There is a discrepancy between multimodal LLMs' general reasoning ability and their weaker performance when they must use external visualization tools and then reason from the tool output. This matters because engineering and scientific workflows often depend on visualization for analysis and decision-making.

Method: The paper introduces VAMPS, a benchmark of 1,168 multimodal, bilingual multiple-choice math QA pairs based on Iranian University Entrance Exam algebra and calculus problems plus human-reviewed LLM-generated synthetic variants. It evaluates whether models can construct useful graphs and reason over the resulting visualizations.

Result: Across a diverse set of models, benchmarking on VAMPS shows that models benefit less than expected from graph-based tool use, and direct symbolic or analytical reasoning performs better than visual-assisted solving.

Conclusion: Direct analytical solving by multimodal large language models unexpectedly outperforms tool-enabled visual solving, even for math problems where plotting should be a natural and helpful strategy.
...
### [347] [StepPRM-RTL: Stepwise Process-Reward Guided LLM Fine-Tuning for Enhanced RTL Synthesis](https://arxiv.org/abs/2606.04246)
...
### [348] [Can Generalist Agents Automate Data Curation?](https://arxiv.org/abs/2606.04261)
...
### [349] [Characterizing initial human-AI proof formalization workflows](https://arxiv.org/abs/2606.04273)
...
### [350] [The Saturation Trap and the Subjectivity of Intervention Timing: Why Affect-Based Triggers and LLM Judges Fail to Time Interventions on Autonomous Agents](https://arxiv.org/abs/2606.04296)
*Manvendra Modgil*
...
### [351] [Exploring Cross-Scenario Generality of Agentic Memory Systems: Diagnostics and a Strong Baseline](https://arxiv.org/abs/2606.04315)
...
### [352] [The Digital Apprentice: A Framework for Human-Directed Agentic AI Development](https://arxiv.org/abs/2606.04321)
...
### [353] [Online Skill Learning for Web Agents via State-Grounded Dynamic Retrieval](https://arxiv.org/abs/2606.04391)
...
### [354] [Not All Errors Are Equal: Consequence-Aware Reasoning Compute Allocation](https://arxiv.org/abs/2606.04402)
...
### [355] [Trivium: Temporal Regret as a First-Class Objective for Causal-Memory Controllers](https://arxiv.org/abs/2606.04421)
*Edward Y. Chang*
...
### [356] [Cascading Hallucination in Agentic RAG: The CHARM Framework for Detection and Mitigation](https://arxiv.org/abs/2606.04435)
*Saroj Mishra*
...
### [357] [The Meta-Agent Challenge: Are Current Agents Capable of Autonomous Agent Development?](https://arxiv.org/abs/2606.04455)
...
### [358] [AgentJet: A Flexible Swarm Training Framework for Agentic Reinforcement Learning](https://arxiv.org/abs/2606.04484)
...
### [359] [Beyond Prompt-Based Planning: MCP-Native Graph Planning-based Biomedical Agent System](https://arxiv.org/abs/2606.04494)
...
### [360] [Simulate, Reason, Decide: Scientific Reasoning with LLMs for Simulation-Driven Decision Making](https://arxiv.org/abs/2606.04505)
...
### [361] [MapAgent: An Industrial-Grade Agentic Framework for City-scale Lane-level Map Generation](https://arxiv.org/abs/2606.04513)
...
### [362] [Scaling Self-Evolving Agents via Parametric Memory](https://arxiv.org/abs/2606.04536)
...
### [363] [Neetyabhas: A Framework for Uncertainty-Aware Public Policy Optimization in Rational Agent-Based Models](https://arxiv.org/abs/2606.04562)
...
### [364] [SCI-PRM: A Tool Aware Process Reward Model for Scientific Reasoning Verification](https://arxiv.org/abs/2606.04579)
...
### [365] [Learning Admissible Heuristics via Cost Partitioning](https://arxiv.org/abs/2606.04597)
...
### [366] [Parthenon Law: A Self-Evolving Legal-Agent Framework](https://arxiv.org/abs/2606.04602)
*Hejia Geng,Leo Liu*
...
### [367] [A Normative Intermediate Representation for ASP-Based Compliance Reasoning](https://arxiv.org/abs/2606.04619)
...
### [368] [MIRAGE: Mobile Agents with Implicit Reasoning and Generative World Models](https://arxiv.org/abs/2606.04627)
...
### [369] [BiNSGPS: Geometry Problem Solving via Bidirectional Neuro-Symbolic Interaction](https://arxiv.org/abs/2606.04648)
...
### [370] [Fog of Love: Engineering Virtuous Agent Behavior with Affinity-based Reinforcement Learning in a Game Environment](https://arxiv.org/abs/2606.04750)
...
### [371] [FALSIFYBENCH: Evaluating Inductive Reasoning in LLMs with Rule Discovery Games](https://arxiv.org/abs/2606.04751)
...
### [372] [Inference-Time Vulnerability Beyond Shallow Safety: Alignment Along Generation Trajectories](https://arxiv.org/abs/2606.04778)
...
### [373] [Tree-Based Formalization of Multi-Agent Complementarity in Human-AI Interactions](https://arxiv.org/abs/2606.04779)
*Andrea Ferrario*
...
### [374] [AIP: A Graph Representation for Learning and Governing Agent Skills](https://arxiv.org/abs/2606.04781)
...
### [375] [BiasGRPO: Stabilizing Bias Mitigation in High-Variance Reward Landscapes via Group-Relative Policy Optimization](https://arxiv.org/abs/2606.04807)
...
### [376] [Beyond Objective Equivalence: Constraint Injection for LLM-Based Optimization Modeling on Vehicle Routing Problems](https://arxiv.org/abs/2606.04816)
...
### [377] [R-APS: Compositional Reasoning and In-Context Meta-Learning for Constrained Design via Reflective Adversarial Pareto Search](https://arxiv.org/abs/2606.04823)
...
### [378] [AICompanionBench: Benchmarking LLMs-as-Judges for AI Companion Safety](https://arxiv.org/abs/2606.04867)
...
### [379] [What Type of Inference is Active Inference?](https://arxiv.org/abs/2606.04935)
...
### [380] [Strabo: Declarative Specification and Implementation of Agentic Interaction Protocols](https://arxiv.org/abs/2606.05043)
...
### [381] [AutoLab: Can Frontier Models Solve Long-Horizon Auto Research and Engineering Tasks?](https://arxiv.org/abs/2606.05080)
...
### [382] [Knowledge Index of Noah's Ark](https://arxiv.org/abs/2606.05104)
